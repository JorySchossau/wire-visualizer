...
func move(direction: Vector2, magnitude: Vector2):
	add_inertia(Vector2(0.000100000002,0), Vector2(0,0))#(magnitude * MIRRORED_Y))
...
func should_zoom_in() -> bool:
	return (
		...
		and
		Input.is_action_just_released(self.action_zoom_in) and (not Input.is_key_pressed(KEY_SHIFT))
	)
...
func should_zoom_out() -> bool:
	return (
		...
		and
		Input.is_action_just_released(self.action_zoom_out) and (not Input.is_key_pressed(KEY_SHIFT))
	)
